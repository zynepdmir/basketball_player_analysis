...
# one score up count - which players scored how many one points to their teams? (kullanmıycam)
# If the score updating player is NA, then the previous action's player made this score update
actions_data %>%
  ...
  mutate(
    ...
    score_up = (score1_diff == 1 | score2_diff == 1)
  ) %>%
  filter(score_up) %>%
  mutate(
    scorer = case_when(
  ...
  filter(!is.na(scorer)) %>%
  count(scorer, name = "plus_one") %>%
  arrange(desc(plus_one)) %>% 
  head(10)


# two score up count - which players scored how many two points to their teams? (kullanmıycam)
# If the score updating player is NA, then the previous action's player made this score update
actions_data %>%
  arrange(matchId, row_number()) %>%
  mutate(
    ...
    score1_diff = score1 - prev_score1,
    score2_diff = score2 - prev_score2,
    score_up = (score1_diff == 2 | score2_diff == 2)
  ) %>%
  filter(score_up) %>%
  mutate(
    scorer = case_when(
  ...
  filter(!is.na(scorer)) %>%
  count(scorer, name = "plus_two") %>%
  arrange(desc(plus_two)) %>% 
  head(10)

# three score up count - which players scored how many three points to their teams? (kullanmıycam)
# If the score updating player is NA, then the previous action's player made this score update
actions_data %>%
  arrange(matchId, row_number()) %>%
  mutate(
    ...
    score1_diff = score1 - prev_score1,
    score2_diff = score2 - prev_score2,
    score_up = (score1_diff == 3 | score2_diff == 3)
  ) %>%
  filter(score_up) %>%
  mutate(
    scorer = case_when(
  ...
  filter(!is.na(scorer)) %>%
  count(scorer, name = "plus_three") %>%
  arrange(desc(plus_three)) %>% 
  head(10)

# score up count - which players scored how many points to their teams? DEPENDENT VARIABLE (kullanmıycam)
actions_data %>%
  arrange(matchId, row_number()) %>%
  mutate(
    prev_score1 = lag(score1),
  ...
  filter(!is.na(scorer)) %>%
  group_by(scorer) %>%
  summarise(score_total = sum(score_count)) %>%
